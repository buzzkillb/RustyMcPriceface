...
class ChartService:
    ...
    def _format_price(self, price: float, is_yield: bool = False) -> str:
        """Format price nicely. Yield tickers (e.g. US10Y) are percentages,
        not USD amounts, so they must not get a '
        ...
        try:
            ...
            ax.annotate(f'{self._format_price(end_price, is_yield)}',
    ...
    async def get_chart_bytes(
        ...
        return self.generate_price_chart(timestamps, prices, crypto.upper(), timeframe_str, hours, is_yield) prefix."""
        if is_yield:
            return f"{price:.3f}%"
        if price >= 10000:
            return f"${price:,.0f}"
