...
class ChartService:
    ...
    def generate_price_chart(
        ...
        try:
            ...
            ax.fill_between(dates, prices_arr, alpha=0.15, color=line_color, zorder=2)
            ...
            ax.annotate(
                f"{self._format_price(end_price)}",
    ...
    def _downsample(
        self, timestamps: list, prices: list, max_points: int = 500
    ) -> tuple:
    ...
    async def get_chart_bytes(
        self, db, crypto: str, hours: int = 24, timeframe_str: str = None
    ) -> Optional[bytes]:
        ...
        timestamps, prices = self._downsample(timestamps, prices, max_points)
        ...
        return self.generate_price_chart(
            timestamps, prices, crypto.upper(), timeframe_str, hours
        )
