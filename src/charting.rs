...
pub fn generate_shanghai_chart(data: &[HistoryData], symbol: &str) -> Result<Vec<u8>, Box<dyn Error + Send + Sync>> {
    let width = 800;
    let height = 400;
...

    {
        ...
        // Find range
        let min_price = data.iter().map(|d| d.shanghai.min(d.western)).fold(f64::INFINITY, f64::min);
        let max_price = data.iter().map(|d| d.shanghai.max(d.western)).fold(f64::NEG_INFINITY, f64::max);
...
pub fn generate_price_chart(data: &[(i64, f64)], symbol: &str) -> Result<Vec<u8>, Box<dyn Error + Send + Sync>> {
    let width = 800;
    let height = 400;
    let mut buffer = vec![0u8; width as usize * height as usize * 3]; // RGB buffer
